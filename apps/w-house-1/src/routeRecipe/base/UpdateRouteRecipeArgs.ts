/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { RouteRecipeWhereUniqueInput } from "./RouteRecipeWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { RouteRecipeUpdateInput } from "./RouteRecipeUpdateInput";

@ArgsType()
class UpdateRouteRecipeArgs {
  @ApiProperty({
    required: true,
    type: () => RouteRecipeWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => RouteRecipeWhereUniqueInput)
  @Field(() => RouteRecipeWhereUniqueInput, { nullable: false })
  where!: RouteRecipeWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => RouteRecipeUpdateInput,
  })
  @ValidateNested()
  @Type(() => RouteRecipeUpdateInput)
  @Field(() => RouteRecipeUpdateInput, { nullable: false })
  data!: RouteRecipeUpdateInput;
}

export { UpdateRouteRecipeArgs as UpdateRouteRecipeArgs };
