/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ProductionOrderWhereUniqueInput } from "./ProductionOrderWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { ProductionOrderUpdateInput } from "./ProductionOrderUpdateInput";

@ArgsType()
class UpdateProductionOrderArgs {
  @ApiProperty({
    required: true,
    type: () => ProductionOrderWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => ProductionOrderWhereUniqueInput)
  @Field(() => ProductionOrderWhereUniqueInput, { nullable: false })
  where!: ProductionOrderWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => ProductionOrderUpdateInput,
  })
  @ValidateNested()
  @Type(() => ProductionOrderUpdateInput)
  @Field(() => ProductionOrderUpdateInput, { nullable: false })
  data!: ProductionOrderUpdateInput;
}

export { UpdateProductionOrderArgs as UpdateProductionOrderArgs };
